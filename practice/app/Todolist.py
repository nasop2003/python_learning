...
def list_add(file_path: str = r"C:\Users\ahsom\Desktop\プログラミング学習\Python\初心者\Todoリスト\TodoList.json" ) :
    ...
    while True:
        ...
        list = input("タスク名: ").strip()

        if list == "": 
            print("文字を入力してください")
            continue
        elif list.isdigit(): #数字が入力されていたら最初から入力
            print("数字は入力できません")
            continue
        elif not list.isalpha(): #記号が入力されていたら最初から入力
            print("記号は入力できません")
            continue
        else:
            break
    ...
    tasks.append({"タスク名": list, "日付": date_now})

    with open(file_path, "w", encoding= "utf-8") as f:
        json.dump(tasks, f, ensure_ascii=False, indent=2)

    print(f"追加しました。タスク名: {list}, 日付: {date_now}")
